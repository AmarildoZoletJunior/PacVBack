...
namespace Booking.Application.Validators
{
    public class BookingRoomValidator : AbstractValidator<BookingRoom>
    {
        ...
        {
            RuleFor(x => x.Start).LessThan(x => x.End).WithMessage("A data de entrada não pode ser maior que a data de saida").GreaterThan(DateTime.Now.AddDays(7)).WithMessage("O quarto só pode ser alugado com adiantamento de 7 dias").NotNull().WithMessage("O campo data de entrada não pode ser nulo").NotEmpty().WithMessage("O campo data de entrada não pode ser vazio");
